...
public class BoardApp {
	...
	public void start() {
		...
		while (run) {
			...
			System.out.println("---------------------------------------------------------");
			System.out.println("1.등록 | 2.목록 | 3.상세조회 | 4.수정 | 5.삭제 | 6.로그아웃 | 9.종료");
			System.out.println("---------------------------------------------------------");
			System.out.println("번호 >>");
	...
	private void login() {
		...
		while (true) {
			...
			failCount++;
			if (failCount >= 3) {
				System.out.println("로그인에 3번 실패하셨습니다.");
				System.out.println("메일 전송중...");
				SendMail sendMail = new SendMail();
				sendMail.sendMail("yeoli.std@gmail.com", "로그인 실패", "로그인에 3번 실패하셨습니다.");
				System.exit(0);
			}
